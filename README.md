...
> **Status:** v1.16.x — data layer, agent layer, frontend, Docker deploy, end-to-end resume + cover-letter tailoring with an **active QA auto-fix loop** that retailors both the resume AND the letter when must-fix issues mention each (closes the v1.10.0 hole where resume-side hallucinations like `~38k LOC` vs verified `~15,700` couldn't be fixed), where **QA receives the verified user-context pool as ground truth** so it can actually validate numeric claims rather than guessing, **per-tailor-chain context-pool refresh** so the LLM always sees today's project state and never cites stale numbers, **non-fatal retry failures** (a schema-violating retry now falls back to the first-pass artefact instead of failing the whole run), and **descriptive PDF filenames cached on the row** — the runs list and the per-job row both render `Jonathan_Duketis-Software_Engineer-SEEK-Resume.pdf` as the link label, with a pinned `<a download=...>` attribute so the save dialog uses the same name regardless of browser quirks. The cross-artefact QA pass works against both pay-per-token API and Claude Pro/Max subscription billing. Backed by **daily auto-discovery of new ATS slugs** from existing apply URLs, a **shared user-context pool** UI proxied through to resumeai (with **snippet + multi-file/folder upload + local git-project scan**), **chat-driven tailor kicks** (the agent has `kick_tailor` / `list_tailor_runs` / `get_tailor_run` tools so you can say "tailor this for me"), **batch + cross-page Select-all** in selection mode (one click queues every matching job across pagination, up to 10k), and **Tailor-from-URL** for any JD URL (paste it; the system matches to the catalogue when it can, or tailors directly when it can't). Generic catalogue (no role bias). **Backend at 100% line + branch coverage; tailor + QA + context UI at 100% line + branch + functions.**
