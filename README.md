...
A local-first AI job-hunting agent for the Australian market. One process scrapes 70+ AU and global job boards on a schedule into a SQLite database, exposes a REST + SSE API, and runs an Anthropic-powered chat agent that uses tools to search and triage roles. The whole thing ships as a single container. Every source scrapes once on startup and then on its cadence (hourly by default), so a restart refreshes the catalogue immediately instead of leaving it stale for up to an hour.

> **Status:** v1.29.x — data layer, agent layer, frontend, Docker deploy, end-to-end resume + cover-letter tailoring with an **active QA auto-fix loop** that retailors both the resume AND the letter when must-fix issues mention each, **per-tailor-chain context-pool refresh** so the LLM always sees today's project state, and a **snapshot-to-disk hook** that drops every successful tailor into a structured `~/jobai-tailored/<Company>-<Title>-<JobID>/` folder (PDFs + JD markdown + QA verdict + metadata + a per-job `CHECKLIST.md` with your apply-profile fields inline + a master `INDEX.md` that flips rows to ✅ when you fill in `Applied on: <date>`). Drag the two PDFs from the folder into the apply form, tick the boxes, move to the next folder — no jobai UI clicking required. The same folder layout is the contract the sibling `interviewai` project reads from for interview prep. The cross-artefact QA pass works against both pay-per-token API and Claude Pro/Max subscription billing. Backed by **daily auto-discovery of new ATS slugs** from existing apply URLs, a **shared user-context pool** UI proxied through to resumeai (with **snippet + multi-file/folder upload + local git-project scan**), **chat-driven tailor kicks** (the agent has `kick_tailor` / `list_tailor_runs` / `get_tailor_run` tools so you can say "tailor this for me"), **batch + cross-page Select-all** in selection mode (one click queues every matching job across pagination, up to 10k), and **Tailor-from-URL** for any JD URL (paste it; the system matches to the catalogue when it can, or tailors directly when it can't). Generic catalogue (no role bias). **Backend at 100% line + branch coverage; tailor + QA + context UI at 100% line + branch + functions.**

## What it does

- **Ingests ~15,000+ jobs per cycle** across 50 ATS sources (Greenhouse, Lever, Ashby, SmartRecruiters, Workable), 15 Seek slugs (every Australian capital + major regional centre + a national remote-only filter), 17 Indeed and 62 LinkedIn slugs (same coverage strategy), 4 AU state government boards (VIC / QLD / SA / WA — fully paginated, ~3,000 jobs/cycle from these alone), and the federal APS Jobs board. The big-board walkers (Seek, Indeed, LinkedIn) walk each query to its natural ceiling rather than stopping at an artificial page cap; multiplying location/work-mode slugs is how we work around the per-query ceilings the boards impose. LinkedIn discovery uses the guest *chunk* endpoint that backs the site's infinite scroll (calibrated live: the plain `/jobs/search` page intermittently serves a sign-in wall, the chunk endpoint reliably paginates 10 cards/request to the ~1,000-per-query ceiling) — and since that ceiling is endpoint-independent **and per-query**, breadth scales with the slug count: now **62** — 8 capitals + 7 regional centres, orthogonal national partitions (work-mode, four recency tiers, the 6 experience levels, four job-types), **and** a big-metro×experience cross (Sydney/Melbourne/Brisbane/Perth/Adelaide × 6 levels) that partitions exactly where the per-city ~1,000 overflow is. All slugs are location/work-mode/recency/level/job-type only — no role keywords — so the catalogue stays generic. A **process-global per-host rate limiter** (in the stealth-tier fetcher stack) paces every request with jitter, so request *rate* — what actually trips LinkedIn's guest fingerprint throttle — stays human regardless of how many slugs run; slug count drives coverage, not detection risk. Cadence is tiered to match: broad-discovery + last-24h slugs hourly, the deep facet partitions every 6h (they're sparse and slow-changing, so re-walking them hourly is wasted load). Completeness over speed by design — a scrape cycle is deliberately slow.
...
- **Normalises `posted_at` to ISO-8601 UTC at canonicalisation.** Boards emit anything from real ISO timestamps (the ATS APIs) to UI labels (`8d ago` from Seek, `4 days ago` / `Just posted` from Indeed) to AU date cells (`14/05/2026` from the state-gov scrapers). A deterministic offline parser resolves every form back to ISO (relative strings measured from scrape time), or NULL when it genuinely can't — so **"Newest posted" sorts by real chronology**, not a lexical string sort over mixed text, and the relative-time labels in the UI are consistent across sources. `jobai normalise-posted` backfills rows scraped before this landed (relative strings resolved against each row's `first_seen_at`).
...
- **Backfills full descriptions** on a slower cadence — LinkedIn guest-mode, Indeed, and Seek detail pages all bypass per-page anti-bot via the tier-3 stealth fetcher. Seek's job-detail SPA never reaches network-idle, so its recipe navigates with `domcontentloaded` and waits on the JD container selector instead of the default `networkidle` wait.
- **Tailors a resume + cover-letter PDF per job, one click.** A new "Tailor" button on every job row kicks an async chain through the two sibling services [resumeai](https://github.com/duketis/resumeai) and [coverletterai](https://github.com/duketis/coverletterai): jobai POSTs the JD (the scraped description text when it has one, otherwise the URL; for any anti-bot-gated or JS-SPA board — Seek, LinkedIn, Indeed, Microsoft Careers (Eightfold) — a host-dispatched resolver fetches the full JD on jobai's own stealth tier first, via each board's detail recipe, so the siblings never hit a 403 / auth wall; non-gated ATS URLs the siblings fetch directly) → resumeai produces a tailored resume → **QA gate 1** grades the resume *alone* against the JD + your verified context and, on must-fix issues, re-tailors the resume and re-grades — **the cover letter is never written until the resume has cleared this gate** → jobai feeds the gated resume's id to coverletterai → coverletterai produces a matching cover letter grounded on that resume → **QA gate 2** grades the letter against the resume + JD + context, catching any letter↔resume contradiction. Each gate emits a structured assessment (coverage / consistency / format scores 0–100, plus must-fix / nice-to-fix lists); the resume gate's verdict is persisted separately from the letter gate's. The QA agent honours the same backend selector as the chat agent — it routes through your Claude Pro/Max quota under `JOBAI_AGENT_BACKEND=subscription` or pay-per-token under `api`, picked up live from the Settings UI without a restart. Both PDFs stream straight back through jobai. A batch mode lets you tick N jobs and queue them all (capped at 3 concurrent chains so the LLM-bound renderers don't pile up). Each gate re-kicks **its own** artefact with the feedback inlined and re-grades — **iterating up to 6 attempts** to converge (gate 1 re-tailors the resume; gate 2 re-tailors the letter only, since the resume is already locked); a single transient retry-render failure no longer aborts a gate (it keeps the last good artefact and keeps trying). The cap is deliberate: an unsatisfiable QA rule can't be allowed to loop forever and burn the API budget, so an artefact that still can't pass after 6 ships best-effort with a **visible FAIL verdict** — never a fake "pass". A dedicated `/tailor-runs` page shows the lifecycle of every chain; the QA verdict shows up as a clickable badge that opens a drill-in panel with the full assessment.
- **Stop a tailor run, and never strand one on restart.** Every in-flight chain shows a **Stop** button (on both the job row and the `/tailor-runs` page) that cancels the in-process task and marks the run failed — no more watching a run hang with no way out. Because chains run as in-process tasks, a container restart would otherwise leave a run "running" forever; a **startup reaper** fails every still-non-terminal run the moment the process boots, so a restart can't strand a run.
- **Re-run, delete, and bulk-clean the run list.** Each finished run on `/tailor-runs` has a **Re-run** button that re-tailors *in place* — it reuses the same row (same job/JD) instead of stacking a new one, so iterating on a job doesn't bloat the list. A two-step **Delete** (arms to "Confirm delete?" so a single misclick can't erase a run) removes one row; a **Select** mode adds per-row checkboxes and a bulk **Delete N selected** so you can clear out the runs you don't care about and keep the ones you do. Deleting an in-flight run cancels its task first so no zombie chain keeps writing to a deleted row.
...
(cd frontend && npm run test:coverage)          # Vitest -- 97 tests, 100% on tailor + QA + context UI
```
